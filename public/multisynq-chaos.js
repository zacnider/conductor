...
class ChaosGameModel extends Multisynq.Model {
    ...
    calculateFinalRankings(room) {
        ...
        // Her oyuncunun claim edeceği CP miktarını göster
        rankings.forEach(ranking => {
            const claimAmount = ranking.score * ranking.multiplier;
            console.log(`💰 ${ranking.playerName} (Rank ${ranking.rank}): ${ranking.score} points × ${ranking.multiplier}x = ${claimAmount} CP tokens`);
        });
...
class ChaosGameView extends Multisynq.View {
    ...
    async handleClaimRewards(ranking) {
        try {
            ...
            if (result.success) {
                // Ödül miktarını hesapla (kontrat ile aynı mantık)
                let multiplier;
                if (rank === 1) multiplier = 5;
                else if (rank === 2) multiplier = 2;
                else if (rank === 3) multiplier = 1.5;
                else multiplier = 1;
                
                const rewardAmount = totalScore * multiplier;
